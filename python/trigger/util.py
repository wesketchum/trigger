...
module = namedtuple("module", ['plugin', 'conf', 'connections', 'extra_commands', 'external_connections_in', 'external_connections_out'], defaults=([], [], []))

app = namedtuple("app", ['modules', 'command_data', 'host'], defaults=({}, "localhost"))
    ...
    for name, mod in modules.items():
        for from_name,to_endpoint in mod.connections.items():
            ...
            if not (found_from or found_to):
                ...
                queue_specs.append(appfwk.QueueSpec(inst=queue_inst, kind='FollyMPMCQueue', capacity=1000))

            if not found_from:
                app_qinfos[name].append(appfwk.QueueInfo(name=from_name, inst=queue_inst, dir="output"))
            if not found_to:
                app_qinfos[to_mod].append(appfwk.QueueInfo(name=to_name, inst=queue_inst, dir="input"))
    ...
    for conn_name, conn in app_connections.items():
        ...
        if from_app == app_name:
            ...
            modules_with_network[qton_name] = module(plugin="QueueToNetwork",
                                                     connections={"input": from_endpoint},
            ...
            for to_conn in conn.subscribers:
                ...
                if app_name == to_app:
                    ...
                    modules_with_network[ntoq_name] = module(plugin="NetworkToQueue",
                                                  connections={"output": to_endpoint},
...
def make_apps_json(apps, app_connections, json_dir):
    ...
    for app_name, app in apps.items():
        modules_plus_network = add_network(app_name, app, app_connections, endpoints)
