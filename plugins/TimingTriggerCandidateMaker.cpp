...
namespace dunedaq {
	namespace trigger {
		...
		TriggerCandidate TimingTriggerCandidateMaker::TimeStampedDataToTriggerCandidate(const TimeStampedData& data) {
			//Fill unused fields
		...
		void TimingTriggerCandidateMaker::do_conf(const nlohmann::json& config) {
...
//			ERS_LOG(get_name() << " successfully configured");
		...
		void TimingTriggerCandidateMaker::init(const nlohmann::json& iniobj) {
		...
		void TimingTriggerCandidateMaker::do_start(const nlohmann::json&) {
			thread_.start_working_thread();
			//ERS_LOG(get_name() << " successfully started");
		}

		void TimingTriggerCandidateMaker::do_stop(const nlohmann::json&) {
			thread_.stop_working_thread();
			//ERS_LOG(get_name() << " successfully stopped");
		}

		void TimingTriggerCandidateMaker::do_work(std::atomic<bool>& running_flag) {
			int receivedCount = 0;
			int sentCount = 0;
		...
		void TimingTriggerCandidateMaker::do_scrap(const nlohmann::json&) {
		}
	} //namespace trigger
} //namespace dunedaq
